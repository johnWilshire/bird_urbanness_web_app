...
const props = {
  ...
  barScale: { type: Number, default: 5 },
  ...
  methods: {
    ...
    updateGraph (source) {
      ...
      const text = allNodes.append('text')
        .attr('dy', '.35em')
        .text(d => {
          return this.showText ? d.data[this.nodeText] : null
        })
        .on('click', d => {
      ...
      var l2 = allNodes.filter(d => {
        return !d.children && d.textInfo
      })
      l2.append('rect')
        .attr('height', 2)
        .attr('width', d => d.data[this.nodeText] * this.barScale)
        .attr('x', d => {
        ...
        .on('mouseover', d => {
          this.$emit('mouseover', { element: d, data: d.data })
          this.mouseovered(true)(d)
        })
        .on('mouseout', d => {
          var vm = this
          setTimeout(() => vm.mouseovered(false)(d), 1000)
        })
    ...
    clean () {
      ['.linktree', '.nodetree', 'text', 'rect', 'circle'].forEach(selector => {
